...
func (GB *GormDB) Init(Param *ParamDB) {
...
func (GB *GormDB) CheckAndCreateTable(table DBModel) {
...
func (GB *gorm.DB) insertCtoDB(ids IdInfo) (err error) {
    if err = GB.Create(ids).Error; err != nil {
    	return
	}
	return
}

func (GB *gorm.DB) updateC(timeString string) (err error) {
	var id IdInfo
	var count int64
	if err1 := GB.Table("id_info").Where("used = ?",true).Count(&count).Error;err1 != nil {
	...
	if err = GB.First(&id, "used = ?",true).Error; err != nil {
		fmt.Println("here1")
		return
	}

    errTs := GB.Transaction(func(trs *gorm.DB) error{
...
func (GB *gorm.DB) updateResult(result, ida string) (err error) {
    if err = GB.Model(IdInfo{}).Where("id_a = ?",ida).Update("id_r",result).Error; err != nil {
    	return
	}
    return
}

func (GB *gorm.DB) CheckC() (checkRes bool) {
	var count int64
	if err := GB.Table("id_info").Where("used = ?",true).Count(&count).Error;err != nil {
		logrus.Println("check the count failed")
		return false
	}
    if count > 0 {
...
func (GB *gorm.DB) GetCids() (err error, ids []string) {
	fmt.Println("coming GetCids----------")
	if err = GB.Model(IdInfo{}).Pluck("id_a",&ids).Error; err != nil {
		return
	}
	return
}

func (GB *gorm.DB) GetCRecord() (err error, record IdInfo) {
	if err = GB.Where("used = true").First(&record).Error; err != nil {
		return
	}
	return
}
...
func (GB *gorm.DB) ClearTable() (err error) {
	if err = GB.Delete(IdInfo{}).Error; err != nil {
...
func (GB *gorm.DB) UpdateUseState(client string) (err error) {
	GB.Transaction(func (trans *gorm.DB) error {
	if err := GB.Model(IdInfo{}).Where("used = true").Update("used","f").Error; err != nil {
		return err
	}
	if err := GB.Model(IdInfo{}).Where("id_a = ? and used = false",client).Update("used","t").Error; err != nil {
		return err
	}
	return nil
	})
